...
## Before to load the train files, we combine the files and get a new dataset:
##
## We load the 'dplyr' package. Remember you must have this installed first!
library(dplyr)
XT <- tbl_df(x_test)
FE <- tbl_df(fea)
...
### In this moment, we have DTS which has the information we need, but only about the test data, we are going
### to use the same approach with the train data. We'll get a new object: DTT, and then we'll merge both to
### get final result
...
YR <- tbl_df(y_train)
# We use the vector with the correct features names to rename the XR's columns
names(XR) <- nm
...
### Previous to merge the two dataframes, DTS and DTR we are going to erase the column id in both (it has no sense
### keep them by now, because every one starts on 1 to... Anyway we can add another one id later, for 1 to n)
DTS <- DTS %>% transmute(id=NULL)
DTR <- DTR %>% transmute(id=NULL)
# Now, we merge the dataframes
DTT <- bind_rows(DTR,DTS)
### As the assignment ask 'the activities'Uses descriptive activity names to name the activities in the data set' I
### drop the "num_act" column
...
### And, we write the dataframe in a file (its name will be 'DataSet5.txt')
